...
pdata_stepniak <- read.csv("pdata/pdata_DA_complete.csv", sep =";", header = TRUE)
...
################################################################################################################################################
#### JIGNA
data_jigna <- read.table("../../../Glioma/Hugnot/data/salmon.merged.gene_counts_Jigna.tsv", header = TRUE)
pdata_jigna <- read.csv("pdata/pdata_Jigna.csv", sep =";", header = TRUE)
...
In this step, we combine the 2 dataframes in order to filter them on the same set of genes and to avoid losing information if we filtered them independently. It was decided to do so after trying to filter them independently and seeing that some genes were filtered out into one dataset but those genes were of interest in the other resulting into a loss of information.
```{r, message=FALSE}
library(HTSFilter)
...
Y_pdata <- read.csv("pdata/pdata.csv", sep =";", header = TRUE)
...
```
### SEPARATION AFTER FILTERING TO NORMALIZE INDEPENDENTLY
After filtering to keep the same set of genes of interest for further analysis, we have to reseparate the datasets in order to perform a normalization. This normalization has to be performed independently for both because each dataset were sequenced independently resulting in different normalization factor in each.
```{r, message=FALSE}
mrna_filter_stepniak <- mrna_filter_common[,9:14]
mrna_filter_jigna <- mrna_filter_common[,1:8]
```
### STEPNIAK DATA NORMALIZATION
We then normalize each dataset independtly using the exact same method used in the differential analysis performed on RNAseq data. (see Differential Analysis files for explanation)
```{r, message=FALSE}
library(mixOmics)
...
```{r, message=FALSE}
#--> Feature annotation
mrna_gene_jigna <- read.table("../../../Glioma/Hugnot/data/salmon.merged.gene_counts_Jigna.tsv", header = TRUE)[,c(1:2)]
...
We then recombine the dataset filtered and normalized to perform the meta analysis
```{r, message=FALSE}
...
This step is just to compare the set of genes remaining with previous one to ensure that no information was lost in the process
```{r, message=FALSE}
library(ggvenn)
genes_Jigna_downreg <- read.csv("../Differential_Analysis/Differential_Analysis_Jigna/lists_genes_up_down_jigna_data/downregulated_genes_Jigna_samples.csv", header = TRUE)
genes_Jigna_upreg <- read.csv("../Differential_Analysis/Differential_Analysis_Jigna/lists_genes_up_down_jigna_data/upregulated_genes_Jigna_samples.csv", header = TRUE)
...
genes_Stepniak_downreg <- read.csv("../Differential_Analysis/Differential_Analysis_Stepniak/RNA/lists_dysreg_genes_stepniak/downregulated_genes_DA.csv", header = TRUE)
genes_Stepniak_upreg <- read.csv("../Differential_Analysis/Differential_Analysis_Stepniak/RNA/lists_dysreg_genes_stepniak/upregulated_genes_DA.csv", header = TRUE)
...
```
### MINT sPLS-DA Model
This step is the meta analysis. It created a model that erases the batch effect between the 2 datasets and that choses features that seem to discriminates between the 2 conditions.
Usually MINT is used to predict the classification of novel samples by generating a predictive model with supervised method. In my analysis, I only wanted to use their method to remove the batch effect to perform in the end a differential analysis and not to create a predictive model.
```{r, message=FALSE}
...
sim <- cim(final_splsda_model, comp = c(1,2), margins=c(10,5), 
    row.sideColors = row_colors,
    legend = legend,
    row.names = TRUE, title = "MINT sPLS-DA, component 1 and 2",
    cut.tree = c(0.1,0.1),
    xlab = "Genes"
    ,save = 'pdf', 
    name.save = "Comp1_and_2_300genes_each_try1"
    )

####################################################################
library(pheatmap)

# Assuming 'sim' is the output from the cim function
# Extract the matrix from cim
heatmap_matrix <- sim$mat

# Perform hierarchical clustering on the samples (columns)
sample_clustering <- hclust(dist(t(heatmap_matrix)))
sample_clusters <- cutree(sample_clustering, k = max(unique(cutree(sample_clustering, h = 0.5))))

# Perform hierarchical clustering on the genes (rows)
gene_clustering <- hclust(dist(heatmap_matrix))
# Set the number of gene clusters to 1 to minimize the number of clusters
gene_clusters <- cutree(gene_clustering, k = 1)

# Define color palette for the class annotations
class_levels <- levels(as.factor(class))
class_colors <- c("#D60000", "#4294D8")
names(class_colors) <- class_levels
...
# Assign colors to samples based on their class
row_colors <- class_colors[class]

# Plot the heatmap
pheatmap(heatmap_matrix,
         cluster_rows = gene_clustering, # Use hierarchical clustering for rows
         cluster_cols = sample_clustering, # Use hierarchical clustering for columns
         cutree_rows = 1, # Minimize gene clusters to 1
         cutree_cols = 1, # Minimize sample clusters to 1
         annotation_col = data.frame(Class = class), # Add sample annotations
         annotation_colors = list(Class = class_colors), # Define colors for annotations
         show_rownames = TRUE, 
         show_colnames = FALSE, 
         main = "Heatmap with Minimized Gene and Sample Clusters")
####################################################################

selected_gene_comp1 <- selectVar(final_splsda_model, comp = 1)$name
selected_gene_comp2 <- selectVar(final_splsda_model, comp = 2)$name

gene_names_comp1 <- mrna_gene_stepniak[match(
  selected_gene_comp1, 
  mrna_gene_stepniak$gene_id),]

gene_names_comp2 <- mrna_gene_stepniak[match(
  selected_gene_comp2, 
  mrna_gene_stepniak$gene_id),]
...
metabo_clust1 <- intersect(cluster1, metabo_genes)

metabo_clust2 <- intersect(cluster2, metabo_genes)

print(paste("Metabolism genes of the first cluster: ", paste(metabo_clust1, collapse = ", ")))
print(paste("Metabolism genes of the second cluster: ", paste(metabo_clust2, collapse = ", ")))
```
### Over representation analysis KEGG metabo
The over representation analysis are the same as presented into the Differential Analysis files.
```{r, message = FALSE}
